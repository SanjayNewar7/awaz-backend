...
class IssueController extends Controller
{
    ...
    {
        try {
            Log::info('Attempting to add reaction to issue', ['issue_id' => $id]);
            ...
            if ($validator->fails()) {
                Log::error('Validation failed for reaction: ' . json_encode($validator->errors()));
                return response()->json([
                    'status' => 'error',
                    'message' => 'Validation failed',
                    'errors' => $validator->errors()
            ...
            if (!$userId) {
                Log::error('Unauthenticated user attempting to react to issue ' . $id);
                return response()->json([
                    'status' => 'error',
                    'message' => 'Unauthenticated'
                ], 401);
